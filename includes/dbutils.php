...
function htmlTable_5( $pdo, $table) {
...
print"<th></th>";
  ...
  foreach( $stmt as $row) {
    ...
    foreach( $row as $v) {
	  ...
	  print("<td><a class='edit_link_3'href='?deletionid_3=".$row['investID']."' class='link'>Delete</a></td>");
...
function deleteRecord_4( $pdo, $deletionid) {
//delete a record from the table list
 $errorMessage = "";
 $stmt = $pdo->prepare("SELECT * FROM `joat_investment_assets` WHERE `investID`=?");
  $stmt->execute( array( $deletionid));
  $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);

  if ( count( $rows) == 1) {
    // delete database record
    $stmt = $pdo->prepare("DELETE FROM `joat_investment_assets` WHERE `investID`=?");
    $stmt->execute( array( $deletionid));
    $affected_rows = $stmt->rowCount();
//header ('location:application.php');
  } else if (count( $rows) > 1) {
